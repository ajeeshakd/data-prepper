...
public class KafkaSourceBuffer<K, V> {
  ...
  public KafkaSourceBuffer(TopicConfig topicConfig, PluginMetrics pluginMetric) {
  ...
  private Record<Object> getEventRecord(final String line, TopicConfig topicConfig) {
  ...
  public void writeRecordToBuffer(String line, final Buffer<Record<Object>> buffer, TopicConfig topicConfig) {
    try {
      buffer.write(getEventRecord(line, topicConfig),
  ...
  public void publishRecordToBuffer(String recordEvent, Buffer<Record<Object>> buffer,
    ...
    try {
      writeRecordToBuffer(recordEvent, buffer, topicConfig);
    } catch (Exception exp) {
  ...
  public void commitOffsets(TopicPartition partition, long lastOffset, KafkaConsumer<K, V> consumer) {
    try {
      if (FALSE.equalsIgnoreCase(topicConfig.getConsumerGroupConfig().getAutoCommit())) {
